  ...
  function BookingsPage() {
    ...
    useEffect(() => {
      axios.get('https://airbnb-clone-mern-server.vercel.app/account/bookings', {
      ...
      <div>
          ...
          {bookings?.length > 0 && bookings.map(booking =>(
            <Link to={`/account/bookings/${booking._id}` } key={booking._id} className='flex gap-4 my-2 overflow-hidden object-fill bg-gray-200 rounded-2xl'>
             ...
             <div className='py-3 '>
                <h2 className='text-sm sm:text-xl font-semibold border-b-2 truncate max-w-full  ' style={{ maxWidth: '160px' }} >{booking.place.title}</h2>
