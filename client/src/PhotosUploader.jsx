...
export const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  async function addPhotoByLink(ev) {
    ...
    try {
      ...
      if (data.imageUrl) {
        const relativeImageUrl = data.imageUrl.replace("https://bookme-server1.onrender.com/uploads", "");
  ...
  function uploadPhoto(ev) {
    ...
    console.log("Photos uploaded:", addedPhotos);
  }
  ...
  return (
    <>
      ...
      <div>
            ...
            addedPhotos.map((link) => (
              <div className="flex relative" key={link}>
                <img
                  ...
                  src={`https://bookme-server1.onrender.com/uploads/${link}`}
                  alt={`Uploaded ${link}`}
                />
        ...
        <label className="h-30 w-30 cursor-pointer grid m-2 border bg-transparent rounded-2xl p-8 text-xl text-gray-600">
          <input
            type="file"
            multiple
            className="hidden"
          ...
          <svg
            ...
            strokeWidth={2}
            stroke="currentColor"
            className="w-5 h-5 m-auto p-auto "
          >
            <path
              strokeLinecap="round"
